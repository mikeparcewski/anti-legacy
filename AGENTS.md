...
Every phase produces exactly one primary artifact. Each producing skill documents that artifact's field schema and done-gate assertions — produce them or flag why you cannot. Full JSON Schemas ship as package data under `antilegacy_core/schemas/` (in the `anti-legacy-expert` skill); the enriched requirements profile is `requirements-graph.enriched.schema.json`, gate-validated. ISS-11: that profile now REQUIRES a numeric `confidence` in [0,1] on every `business_rules` item, so a confidence-less or non-numeric-confidence rule is a HARD GATE_1_DESIGN validation failure (not merely a risk-log warning); confidence stays OPTIONAL on validation/error-path items.
...
| `precheck` | Producer-side readiness gate (`precheck <phase> [--advisory] [--json] [--strict]`): per-phase completeness predicates + disk-reality reconcile; producers call `require_ready(phase, force=, strict=)` and REFUSE (exit 1) on an incomplete/orphaned/state-desynced pipeline. `--strict` / `PRECHECK_STRICT` (ISS-22) escalates the no-profile catch-all from warn-pass to BLOCK so a new producer can't skip readiness gating. Read-only — never writes audit.jsonl, advances a phase, or clears a gate | Clearing or recording a gate (that is `manifest gate`) |
