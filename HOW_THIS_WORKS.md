...
One engine indexes the whole estate — the mainframe stack (COBOL/JCL/CICS/IMS/DB2) and modern languages (Java, C#, Python, TypeScript, Go, Rust, …) in the same pass, token-free even across thousands of files. There is no language-routing split and no batch Python extractor; `survey-modern` survives only as a thin find+grep fallback for source the engine cannot index.
