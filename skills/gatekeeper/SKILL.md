...
| `GATE_3C_DIFFERENTIAL` | After build, when a golden corpus exists | differential-equivalence-report (`gate_posture` PASS / WARN / NOT_APPLICABLE all clear; only BLOCK fails). Record the report's `golden_confidence` in the rationale (ISS-25) — never a bare "passed" | Automated — **provenance-graded warning**, not a hard gate unless the golden is **attested** captured-legacy. A parity FAIL against a low/medium-confidence golden WARNS (the golden may itself be wrong); only a FAIL against an attested `captured-legacy` golden BLOCKS → kick back to `build`. `golden_confidence` reaches `high` (BLOCK-reachable) only when each captured-legacy entry carries a `capture` attestation `{method, source, captured_at}` (ISS-24) — a bare label is auto-demoted to low/WARN. Vacuous-safe; NOT in the advance-precondition map |
