...
def cmd_init(args):
    ...
    # manifest.json ships as package data inside antilegacy_core, so it travels
    # with the bundle wherever skills.sh installs it. Requires running as
    # `python -m antilegacy_core.manifest` (which run.py does).
    _template_text = (
        resources.files("antilegacy_core") / "templates" / "manifest.json"
    ).read_text()
    ...
    manifest = json.loads(_template_text)
